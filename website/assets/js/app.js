...
(function () {
  ...
  (function () {
    ...
    function track(event, data) {
      ...
      /* ЕДИНСТВЕНИЯТ маршрут към GA4: този push. Тригерът „Custom
         Event“ в GTM го хваща, GA4 Event тагът го праща нататък.

         Тук стоеше и `window.gtag('event', event, payload)`. Изглеждаше
         безобиден, защото gtag() също само бута в dataLayer — но точно
         затова не беше: GTM разпознава и gtag-командите като събития.
         Едно действие палеше тригера ДВА пъти и в GA4 влизаха по две
         копия. Мълчеше, докато в контейнера нямаше GA4 Event таг, и
         проговори в деня, в който той се появи.

         Ако някога GA4 трябва да получава събития БЕЗ GTM, това става
         с отделен gtag.js loader — и тогава единият от двата маршрута
         се изключва. Никога и двата едновременно. */
