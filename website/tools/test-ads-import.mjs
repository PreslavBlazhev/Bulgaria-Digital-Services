...
/* ============================================================
   5б. Външното състояние е записано, не предположено
   ============================================================ */
G('Блокерът е документиран честно');

const adsDoc = read('marketing/ad-data-automation.md');
check('Google е отбелязан като без акаунт',
  /BLOCKED — NO REAL GOOGLE ADS ACCOUNT/.test(adsDoc));
check('Meta е отбелязана като без акаунт',
  /BLOCKED — NO REAL META AD ACCOUNT/.test(adsDoc));
check('записано е кога и от кого е потвърдено',
  /потвърдено от собственика, 28\.08\.2026/.test(adsDoc));
check('описани са точните стъпки за деня, в който има акаунт',
  /Bulk actions → Scripts/.test(adsDoc) && /SPREADSHEET_URL/.test(adsDoc));
check('никъде не се твърди, че има внесени данни',
  !/внесени реални данни|данните са внесени/i.test(adsDoc));
check('липсата на рекламен акаунт не се води блокер за 16/17/18/20',
  /не.{0,3} спира.{0,60}Phase 16, 17, 18/s.test(adsDoc));
